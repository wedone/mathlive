...
默认布局为许多按键提供变体。通过长按按键可以访问这些变体。变体通常是与主按键相关但更少使用的版本。

要为自定义布局定义变体，请在按键定义中指定 `variants` 属性。该属性值是 `VirtualKeyboardKeycap` 数组。作为快捷方式，也可以直接使用字符串，等同于 `latex` 属性为该字符串的按键记录。
